...
def register(request):
	if request.method == "POST":
		...
		current_site = get_current_site(request) # Pour recuperer le site
		mail_subject = "S'il vous plait, activez votre compte !!!" # sujet du mail
		# Qui contient le message
		message = render_to_string('accounts/account_verification_email.html', {
			'user': user, # utilisateur
			'domain': current_site, # addresse du site
			'uid': urlsafe_base64_encode(force_bytes(user.pk)), # uid
			'token': default_token_generator.make_token(user), #token genere
		})
		to_email = email # Email du destinataire
		send_email = EmailMessage(mail_subject, message, to=[to_email]) # mail a envoyer
